...
# $Id: cov-match.R 1492 2010-08-26 03:05:40Z mthompson $
...
cov.match.sample <- function(target.dist, x0, sample.size, tuning=1, theta=1,
  ...
  twonorm <- function(v) sqrt(sum(v^2))
  nevals <- 0
  ngrads <- 0
  adaptations <- 0          # crumbs drawn where alpha>0
  non.adaptations <- 0      # crumbs drawn where alpha==0
  p <- length(x0)
  X <- array(NA,c(sample.size,p))
  X[1,] <- x0

  # Loop that generates sample.size-1 observations from target distribution

  for (obs in 2:sample.size) {
    y.max <- -Inf                       # max. log density so far this obs.
    x0 <- t(X[obs-1,,drop=FALSE])       # state we are starting from
    nevals <- nevals + 1
    y0 <- target.dist$log.density(x0)   # log density at start state
    if (y0>y.max)
      y.max <- y0
	...
	y.max <- y1
      ...
      if (alpha>0) {
        ...
        non.adaptations <- non.adaptations + 1
      }
    }
  ...
  adapt.rate <- adaptations/(adaptations+non.adaptations)
  return(list(X=X, adapt.rate=adapt.rate, evals=nevals, grads=ngrads))
}
